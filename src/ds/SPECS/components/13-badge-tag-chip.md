...
> Spec 13 · grupo: `primitives` · referência visual: `reference-html/Component - Badge, Tag, Chip.html`

## Resumo

3 componentes relacionados: Badge (status pill, no interaction), Tag (categorical, removable), Chip (input, deletable + clickable). Cada um com variantes neutral/info/success/warning/danger/brand. Sizes sm/md.

## Onde fica

`src/components/primitives/BadgeTagChip/`
...
BadgeTagChip/
├── BadgeTagChip.tsx
├── BadgeTagChip.stories.tsx
├── BadgeTagChip.test.tsx
└── index.ts
```

## Como implementar

1. **Abra o HTML de referência** em `reference-html/Component - Badge, Tag, Chip.html` no navegador.
...
```tsx
import { BadgeTagChip } from '@/components';
