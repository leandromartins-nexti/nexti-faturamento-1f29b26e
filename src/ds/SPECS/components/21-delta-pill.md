...
> Spec 21 · grupo: `containers` · referência visual: `reference-html/Component - Delta Pill.html`

## Resumo

Pequena pill de variação. Auto-color (positivo=success, negativo=danger; invertível via prop "good": "up"|"down"). Format: prefix arrow + número + opcional suffix unit. Variants: subtle (bg) / strong (filled) / ghost (only color).

## Onde fica

`src/components/containers/DeltaPill/`
...
1. **Abra o HTML de referência** em `reference-html/Component - Delta Pill.html` no navegador.
...
```tsx
import { DeltaPill } from '@/components';
...
- [ ] Re-export adicionado em `src/components/containers/index.ts`
