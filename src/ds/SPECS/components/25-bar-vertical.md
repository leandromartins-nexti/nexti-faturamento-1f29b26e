...
> Spec 25 · grupo: `charts` · referência visual: `reference-html/Component - Bar Chart Vertical.html`

## Resumo

Single + grouped + stacked. Eixo X categorical, Y numérico. Tooltip via Radix Tooltip. Color por série (de tokens.color.series). Empty + loading states via ChartContainer.

## Onde fica

`src/components/charts/BarChartVertical/`
...
├── BarChartVertical.stories.tsx
├── BarChartVertical.test.tsx
└── index.ts
```

## Como implementar

1. **Abra o HTML de referência** em `reference-html/Component - Bar Chart Vertical.html` no navegador.
...
```tsx
import { BarChartVertical } from '@/components';
...
- [ ] Re-export adicionado em `src/components/charts/index.ts`
