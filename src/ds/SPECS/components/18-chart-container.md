...
> Spec 18 · grupo: `containers` · referência visual: `reference-html/Component - Chart Container.html`

## Resumo

Wrapper canônico de qualquer chart. Slots: title, subtitle, eyebrow (kpi), filters (top-right), body (chart), footer (legenda + meta). Skeleton automático via prop loading. Hero variant (mais alto, padding maior). Empty state integrado.

## Onde fica

`src/components/containers/ChartContainer/`
...
1. **Abra o HTML de referência** em `reference-html/Component - Chart Container.html` no navegador.
