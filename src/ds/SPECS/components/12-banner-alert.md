...
> Spec 12 · grupo: `primitives` · referência visual: `reference-html/Component - Banner Alert.html`

## Resumo

Inline alert (não dismissible automático). 4 intents. Icon (auto pelo intent ou custom). Optional dismiss button + action button. Compact + full variants.

## Onde fica

`src/components/primitives/BannerAlert/`
...
BannerAlert/
├── BannerAlert.tsx
├── BannerAlert.stories.tsx
├── BannerAlert.test.tsx
└── index.ts
```

## Como implementar

1. **Abra o HTML de referência** em `reference-html/Component - Banner Alert.html` no navegador.
