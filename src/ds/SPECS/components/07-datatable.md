...
> Spec 07 · grupo: `primitives` · referência visual: `reference-html/Component - DataTable.html`

## Resumo

Sortable, selectable, paginated table. Columns: definition object {key, header, render, sortable, width, align}. Row selection (single/multi via Checkbox). Sticky header. Empty state. Loading via Skeleton rows. Density (cozy/comfortable/compact).

## Onde fica

`src/components/primitives/DataTable/`
...
1. **Abra o HTML de referência** em `reference-html/Component - DataTable.html` no navegador.
