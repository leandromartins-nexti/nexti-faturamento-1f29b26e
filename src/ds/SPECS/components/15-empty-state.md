...
> Spec 15 · grupo: `primitives` · referência visual: `reference-html/Component - Empty State.html`

## Resumo

Illustration + título + descrição + 1-2 CTAs. 3 variants (no-data, no-results, error). Compact (inline) + full (centered) layouts.

## Onde fica

`src/components/primitives/EmptyState/`
...
1. **Abra o HTML de referência** em `reference-html/Component - Empty State.html` no navegador.
